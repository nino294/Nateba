...
const AuthModal=({mode:initMode,lang,onAuth,onClose})=>{
  ...
  const signInGoogle=async()=>{setGLoading(true);setErr("");try{const result=await signInWithPopup(fbAuth,googleProvider);const u=result.user;const{data,error}=await supabase.from("users").upsert({firebase_uid:u.uid,phone:u.uid,name:u.displayName||u.email.split("@")[0],email:u.email},{onConflict:"firebase_uid"}).select().single();if(error)console.error("Supabase:",error);const{data:fresh}=await supabase.from("users").select("role,id").eq("firebase_uid",u.uid).single();onAuth({name:u.displayName||u.email.split("@")[0],email:u.email,uid:u.uid,role:fresh?.role||data?.role||"student",id:fresh?.id||data?.id});}catch(e){if(e.code!=="auth/popup-closed-by-user"&&e.code!=="auth/cancelled-popup-request")setErr(getErr(e.code));}setGLoading(false);};
  const handleSubmit=async()=>{setErr("");if(!email||!pw){setErr(isKa?"შეავსე ყველა ველი":"Please fill in all fields");return;}if(mode==="signup"&&!name){setErr(isKa?"შეიყვანე სახელი":"Please enter your name");return;}if(mode==="signup"&&pw!==pw2){setErr(isKa?"პაროლები არ ემთხვევა":"Passwords do not match");return;}if(mode==="signup"&&pw.length<6){setErr(isKa?"პაროლი მინიმუმ 6 სიმბოლო":"Password must be 6+ characters");return;}setLoading(true);try{const{createUserWithEmailAndPassword,signInWithEmailAndPassword,updateProfile}=await import("firebase/auth");let fu;if(mode==="signup"){const c=await createUserWithEmailAndPassword(fbAuth,email,pw);fu=c.user;await updateProfile(fu,{displayName:name});}else{const c=await signInWithEmailAndPassword(fbAuth,email,pw);fu=c.user;}const displayName=mode==="signup"?name:(fu.displayName||email.split("@")[0]);const upsertData=mode==="signup"?{firebase_uid:fu.uid,phone:fu.uid,name:displayName,email,role}:{firebase_uid:fu.uid,phone:fu.uid,name:displayName,email};const{data,error}=await supabase.from("users").upsert(upsertData,{onConflict:"firebase_uid"}).select().single();if(error)console.error("Supabase:",error);const{data:fresh}=await supabase.from("users").select("role,id").eq("firebase_uid",fu.uid).single();onAuth({name:displayName,email,uid:fu.uid,role:fresh?.role||data?.role||role,id:fresh?.id||data?.id});}catch(e){console.error(e.code,e.message);setErr(getErr(e.code));}setLoading(false);};
  return(
