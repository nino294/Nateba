...
const AuthModal=({mode:initMode,lang,onAuth,onClose})=>{
  ...
  const signInGoogle=async()=>{setGLoading(true);setErr("");try{const result=await signInWithPopup(fbAuth,googleProvider);const u=result.user;let userRole="student";let userId=null;const{data:existing}=await supabase.from("users").select("role,id").eq("firebase_uid",u.uid).single();if(existing){userRole=existing.role||"student";userId=existing.id;await supabase.from("users").update({name:u.displayName||u.email.split("@")[0],email:u.email}).eq("firebase_uid",u.uid);}else{const{data:newU}=await supabase.from("users").insert({firebase_uid:u.uid,phone:u.uid,name:u.displayName||u.email.split("@")[0],email:u.email,role:"student"}).select().single();if(newU){userRole=newU.role||"student";userId=newU.id;}}onAuth({name:u.displayName||u.email.split("@")[0],email:u.email,uid:u.uid,role:userRole,id:userId});}catch(e){if(e.code!=="auth/popup-closed-by-user"&&e.code!=="auth/cancelled-popup-request")setErr(getErr(e.code));}setGLoading(false);};
  const handleSubmit=async()=>{setErr("");if(!email||!pw){setErr(isKa?"შეავსე ყველა ველი":"Please fill in all fields");return;}if(mode==="signup"&&!name){setErr(isKa?"შეიყვანე სახელი":"Please enter your name");return;}if(mode==="signup"&&pw!==pw2){setErr(isKa?"პაროლები არ ემთხვევა":"Passwords do not match");return;}if(mode==="signup"&&pw.length<6){setErr(isKa?"პაროლი მინიმუმ 6 სიმბოლო":"Password must be 6+ characters");return;}setLoading(true);try{const{createUserWithEmailAndPassword,signInWithEmailAndPassword,updateProfile}=await import("firebase/auth");let fu;if(mode==="signup"){const c=await createUserWithEmailAndPassword(fbAuth,email,pw);fu=c.user;await updateProfile(fu,{displayName:name});}else{const c=await signInWithEmailAndPassword(fbAuth,email,pw);fu=c.user;}const displayName=mode==="signup"?name:(fu.displayName||email.split("@")[0]);let userRole=role;let userId=null;if(mode==="login"){const{data:existing}=await supabase.from("users").select("role,id").eq("firebase_uid",fu.uid).single();if(existing){userRole=existing.role||"student";userId=existing.id;await supabase.from("users").update({name:displayName,email}).eq("firebase_uid",fu.uid);}else{const{data:newU}=await supabase.from("users").insert({firebase_uid:fu.uid,phone:fu.uid,name:displayName,email,role:"student"}).select().single();if(newU){userRole=newU.role||"student";userId=newU.id;}}}else{const{data:newU}=await supabase.from("users").insert({firebase_uid:fu.uid,phone:fu.uid,name:displayName,email,role}).select().single();if(newU){userRole=newU.role||role;userId=newU.id;}}onAuth({name:displayName,email,uid:fu.uid,role:userRole,id:userId});}catch(e){console.error(e.code,e.message);setErr(getErr(e.code));}setLoading(false);};
  return(
